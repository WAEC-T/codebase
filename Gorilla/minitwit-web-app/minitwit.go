...
func get_user_id(username string) (any, error) {
	var user_id int
	rv := db.QueryRow("SELECT user_id FROM public.users WHERE username = $1",
		username)
	err := rv.Scan(&user_id)
	if err != nil {
		return nil, err
...
func follow_user(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec("INSERT INTO public.followers (who_id, whom_id) VALUES ($1, $2)", user_id, whom_id)
	if err != nil {
...
func unfollow_user(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec("DELETE FROM public.followers WHERE who_id=$1 and whom_id=$2", user_id, whom_id)
	if err != nil {
...
func timeline(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || isNil(user) {
        ...
        WHERE public.messages.flagged = 0 AND public.messages.author_id = public.users.user_id AND (
            public.users.user_id = $1 OR
            public.users.user_id IN (SELECT whom_id FROM public.followers
                                    where who_id = $2))
        ORDER BY public.messages.pub_date desc limit $3`
...
func user_timeline(w http.ResponseWriter, r *http.Request) {
	...
	following := getFollowing(user_id)
	profile_user, err := query_db("SELECT * FROM public.users WHERE username = $1", []any{username}, true)
	...
	var query = `SELECT public.messages.*, public.users.* FROM public.messages, public.users WHERE
	public.users.user_id = public.messages.author_id AND public.users.user_id = $1
	ORDER BY public.messages.pub_date desc limit $2`
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil && !(isNil(user)) {
		...
		user, err := query_db("select * from public.users where username = $1", []any{username}, true)
		if err != nil || isNil(user) {
...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil && !(isNil(user)) {
		...
		if username == "" {
			...
			_, err = db.Exec("INSERT INTO public.users (username, email, pw_hash) VALUES ($1, $2, $3)", username, email, hashedPassword)
			if err != nil {
...
func getFollowing(userId any) any {
	...
	usr, err := query_db(`select whom_id from public.followers where
	public.followers.who_id = $1`, []any{userId}, false)
